    ...
    <div className="talabat-section animate-fade-in animate-delay-3">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-4">
          {Array(isMobile ? 2 : 4).fill(0).map((_, i) => (
        ...
        <div className="grid grid-cols-1 gap-4">
          {promos.map((promo, index) => {
            // تخصيص ألوان أفضل لكل خدمة
            const gradients = {
              'restaurants': 'from-orange-500 to-orange-700',
              'market': 'from-emerald-500 to-emerald-700',
              'pharmacy': 'from-blue-500 to-blue-700',
              'personal-care': 'from-pink-500 to-purple-600',
            ...
            const iconBgs = {
              ...
              'pharmacy': 'bg-blue-400/30',
              'personal-care': 'bg-pink-400/30',
              'gym': 'bg-indigo-400/30'
            };

            // حدد تدرج اللون المناسب بناءً على نوع الخدمة
            const path = promo.link.split('/').filter(p => p).pop() || '';
            const gradient = gradients[path as keyof typeof gradients] || 'from-orange-500 to-orange-700';
            const iconBg = iconBgs[path as keyof typeof iconBgs] || 'bg-white/30';
            ...
            return (
              ...
              >
                <CardContent className="p-0 relative">
                  {/* تحسين تدرج الخلفية */}
                  <div className={`absolute inset-0 bg-gradient-to-r ${gradient} opacity-95 rounded-xl`}></div>
                  
                  <div className="relative p-5 flex items-start justify-between">
                    <div className="flex flex-col items-start w-full">
                      <div className="flex items-center justify-between w-full mb-3">
                        <div className={`${iconBg} p-3 rounded-xl shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
                          {getIcon(promo.icon)}
                        </div>
                        ...
                        <Button 
                          size="sm" 
                          variant="secondary"
                          className="bg-white/30 hover:bg-white/50 transition-colors text-white border-none group-hover:translate-x-1 transition-transform duration-300 mr-2"
                        >
                          استكشف <ArrowRight className="mr-1 h-4 w-4 rtl:rotate-180" />
                        </Button>
                      </div>
