...
const PopularPlaces: React.FC = () => {
  ...
  return <div className="px-4 mb-8 animate-fade-in animate-delay-2">
      ...
      <div className="scroll-container">
        {places.map((place, index) => <Card key={place.id} onClick={() => navigateToRestaurant(place.id)} className="w-72 cursor-pointer overflow-hidden border-blue-100 hover:border-blue-300 hover:shadow-md shadow-sm transition-all duration-300 flex-shrink-0 animate-fade-in" style={{
        animationDelay: `${index * 100}ms`
      }}>
            <div className="relative h-44">
              <img src={place.image} alt={place.name} className="w-full h-full object-cover" loading="lazy" />
              {place.discount && <Badge className="absolute top-3 left-3 bg-blue-600 text-white border-0">
                  <BadgePercent className="w-3.5 h-3.5 mr-1" /> {place.discount}
                </Badge>}
              {place.isNew && <Badge className="absolute top-3 left-3 bg-green-600 text-white border-0">
                  جديد
                </Badge>}
            ...
            <CardContent className="p-3">
              ...
              <Button className="w-full mt-3 bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white shadow-sm" size="sm" onClick={e => {
            e.stopPropagation();
