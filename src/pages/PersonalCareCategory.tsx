      ...
      <div className="max-w-md mx-auto bg-white pb-20">
        ...
        <div className="flex items-center justify-between p-4 bg-white shadow-sm sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/personal-care/cart" className="relative text-gray-700">
              ...
              {itemCount > 0 && <span className="absolute -top-2 -right-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">
                  {itemCount}
                </span>}
            </Link>
          </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="relative">
            <input type="text" placeholder={`ابحث في ${currentCategory.name}...`} className="w-full p-3 pl-10 pr-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
            <Search className="absolute left-3 top-3 text-gray-400 w-5 h-5" />
          </div>
          
          {/* Filter button */}
          <div className="flex justify-between items-center">
            <Button variant="outline" className="border-gray-300 flex items-center gap-2" onClick={() => setShowFilters(!showFilters)}>
          ...
          {/* Filter options */}
          {showFilters && <div className="bg-gray-50 p-4 rounded-lg animate-fade-in">
              <h3 className="font-medium mb-3">ترتيب حسب:</h3>
              <div className="flex flex-wrap gap-2">
                {filterOptions.map(option => <Button key={option.id} variant={activeFilter === option.id ? "default" : "outline"} size="sm" onClick={() => setActiveFilter(option.id)} className={activeFilter === option.id ? "bg-gradient-to-r from-purple-500 to-pink-500" : "bg-white"}>
                    {option.name}
                  </Button>)}
              </div>
              
              <h3 className="font-medium mb-3 mt-4">السعر:</h3>
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" className="bg-white">أقل من 50 ريال</Button>
                <Button variant="outline" size="sm" className="bg-white">50 - 100 ريال</Button>
                <Button variant="outline" size="sm" className="bg-white">100+ ريال</Button>
              </div>
            </div>}
          
          {/* Horizontal Filter Tabs */}
          <div className="flex overflow-x-auto gap-2 pb-2 no-scrollbar -mx-1 px-1">
            {filterOptions.map(option => <Button key={option.id} variant={activeFilter === option.id ? "default" : "outline"} size="sm" className={`whitespace-nowrap ${activeFilter === option.id ? "bg-gradient-to-r from-purple-500 to-pink-500" : "border-gray-200"}`} onClick={() => setActiveFilter(option.id)}>
                {option.name}
              </Button>)}
          </div>
        </div>
        ...
        <div className="p-4">
          {loading ? <div className="grid grid-cols-2 gap-4">
              ...
              {products.map(product => <Card key={product.id} className="overflow-hidden border border-gray-100 hover:shadow-md transition-shadow">
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <Button size="sm" className="rounded-full h-7 w-7 p-0 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 shadow-sm" onClick={() => handleAddToCart(product)}>
        ...
        {itemCount > 0 && <Link to="/personal-care/cart">
            <div className="fixed bottom-5 left-0 right-0 mx-auto w-4/5 max-w-md bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full py-3 px-5 flex items-center justify-between shadow-lg my-[51px]">
