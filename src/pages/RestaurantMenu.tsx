      ...
      <div className="max-w-md mx-auto bg-white pb-24">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-black/70 to-transparent">
            <div className="flex justify-between items-start p-4">
              <button onClick={() => navigate('/restaurants')} className="p-2 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/40 transition-colors">
                <ArrowLeft className="w-5 h-5 text-white" />
              </button>
              <div className="flex gap-2">
                <button onClick={shareRestaurant} className="p-2 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/40 transition-colors">
                  <Share2 className="w-5 h-5 text-white" />
                </button>
                <button onClick={toggleFavorite} className="p-2 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/40 transition-colors">
        ...
        <div className="relative px-4 pb-4 border-b">
          <div className="flex items-start mt-3">
            <img src={restaurant.logo} alt={restaurant.name} className="w-20 h-20 rounded-lg border-4 border-white shadow-sm object-cover -mt-10" />
        ...
        <div className="border-b sticky top-0 bg-white z-10 shadow-sm">
          <div className="flex overflow-x-auto gap-2 py-3 px-4 no-scrollbar">
            {restaurant.categories.map(category => <Button key={category.id} variant={selectedCategory === category.id ? "default" : "outline"} className={`whitespace-nowrap rounded-full transition-all ${selectedCategory === category.id ? 'bg-brand-500 text-white shadow-md' : 'text-gray-700 border-gray-200 hover:bg-gray-50'}`} onClick={() => setSelectedCategory(category.id)}>
                {category.name}
              </Button>)}
          </div>
        </div>
        ...
        <div className="px-4 py-3">
          {categoryHasItems ? filteredItems.map(item => <div key={item.id} className="flex gap-3 border-b py-4 group hover:bg-gray-50 transition-colors rounded-lg px-2">
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900">{item.name}</h3>
                  <p className="text-sm text-gray-600 mt-1 line-clamp-2">{item.description}</p>
                  <div className="flex items-center justify-between mt-3">
                    <span className="font-bold text-gray-900">{item.price} ريال</span>
                    <Button size="sm" onClick={() => addToCart(item)} className="rounded-full shadow-sm group-hover:bg-primary group-hover:scale-105 transition-all">
                ...
                <div className="relative w-28 h-28 flex-shrink-0">
                  <img src={item.image} alt={item.name} className="w-full h-full rounded-lg object-cover shadow-sm group-hover:shadow-md transition-shadow" />
                  {item.offer && <Badge className="absolute top-2 right-2 text-xs bg-red-500 shadow-sm">
                      {item.offer}
                    </Badge>}
                </div>
        ...
        {cartItems.length > 0 ? <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 max-w-md mx-auto shadow-lg animate-fade-in">
            <Button onClick={() => navigate('/cart')} className="w-full py-6 text-lg rounded-xl bg-brand-500 shadow-md hover:shadow-lg transition-shadow bg-lime-400 hover:bg-lime-300 text-slate-800">
            ...
            <Button className="w-full py-6 text-lg rounded-xl bg-gray-100 text-gray-400 cursor-not-allowed" disabled>
              <ShoppingBag className="mr-2" />
              اختر طعامك المفضل
            </Button>
          </div>}
