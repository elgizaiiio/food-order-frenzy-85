      ...
      <div className="max-w-md mx-auto bg-white pb-20">
            ...
            <Popover>
              ...
              <PopoverContent className="w-60" align="start">
                <div className="space-y-2">
                  ...
                  {savedAddresses.map((addr, idx) => <Button key={idx} variant="ghost" className="w-full justify-start text-sm" onClick={() => setAddress(addr)}>
        ...
        <div className="px-4 py-3 border-b">
          <div className="flex overflow-x-auto gap-4 pb-3 no-scrollbar">
            {foodCategories.map(category => <div key={category.id} className={`flex flex-col items-center flex-shrink-0 cursor-pointer transition-all`} onClick={() => toggleCategoryFilter(category.id)}>
                <div className={`food-category-circle flex items-center justify-center w-16 h-16 rounded-full mb-1 transition-colors ${activeFilterCategory === category.id ? 'bg-primary/10 border-2 border-primary' : 'bg-gray-100'}`}>
                  <span className="text-2xl">{category.icon}</span>
                </div>
                <span className={`text-xs font-medium ${activeFilterCategory === category.id ? 'text-primary' : 'text-gray-700'}`}>
                  {category.name}
                </span>
              </div>)}
          </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {restaurant.tags && restaurant.tags.map((tag, idx) => <span key={idx} className="text-xs text-gray-500">{tag}{idx !== restaurant.tags.length - 1 && ' • '}</span>)}
                          </div>
                        </div>
                      </div>
                    </div>
