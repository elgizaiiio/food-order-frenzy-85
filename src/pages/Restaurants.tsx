        ...
        <div className="px-4 py-3 border-b">
          <div className="flex overflow-x-auto gap-4 pb-3 no-scrollbar">
            {foodCategories.map((category) => (
              ...
              >
                <div 
                  className={`food-category-circle flex items-center justify-center w-16 h-16 rounded-full mb-1 transition-colors ${
                    activeFilterCategory === category.id ? 'bg-primary/10 border-2 border-primary' : 'bg-gray-100'
                  }`}
                >
                  <span className="text-2xl">{category.icon}</span>
                </div>
                <span className={`text-xs font-medium ${
                  activeFilterCategory === category.id ? 'text-primary' : 'text-gray-700'
                }`}>
                  {category.name}
                </span>
              </div>
          ...
          {filters.map((filter) => (
            <Button 
              ...
              className={`rounded-full whitespace-nowrap ${
                activeSortFilter === filter.id ? 'bg-primary' : ''
              }`}
