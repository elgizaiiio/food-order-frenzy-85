...
const DamMarketContent: React.FC = () => {
  ...
  return <div className="min-h-screen bg-blue-50" dir="rtl">
      <div className="max-w-md mx-auto bg-white pb-24">
        ...
        {/* Intro Banner */}
        <div className="mx-4 my-4 p-5 rounded-2xl bg-gradient-to-r from-blue-100 to-blue-50 border border-blue-200 shadow-sm">
          <h2 className="text-2xl font-bold text-blue-800 mb-2">توصيل سريع لمنتجاتك اليومية</h2>
          ...
          {offersLoading ? <div className="overflow-x-auto flex gap-3 pb-2">
              ...
              {offers?.map(offer => <Card key={offer.id} className="min-w-60 overflow-hidden flex-shrink-0 border-0 rounded-xl shadow-md animate-fade-in">
                  ...
                  <div className="p-3 bg-gradient-to-b from-blue-50 to-white">
                    <h4 className="font-bold text-blue-800">{offer.title}</h4>
                    <p className="text-xs text-gray-600 mb-2">{offer.description}</p>
                    <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl shadow-sm">
                      استفد الآن
                    </Button>
                  </div>
                </Card>)}
